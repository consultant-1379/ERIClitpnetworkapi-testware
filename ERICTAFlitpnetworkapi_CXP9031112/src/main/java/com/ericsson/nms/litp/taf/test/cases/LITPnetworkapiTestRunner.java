...
public class LITPnetworkapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPnetworkapiTestRunner.class);
    ...
    @TestId(id = "CXP9031112-1", title = "Upgrade ERIClitpnetworkapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpnetworkapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("networkapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031112-2", title = "Run python test cases for ERIClitpnetworkapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpnetworkapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpnetworkapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031112-3", title = "Parse xml outputs from python tests")
